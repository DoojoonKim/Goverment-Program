...
# 1. 데이터 전처리
# - 문자데이터를 수치화
# - NaN(Null) 값을 어떻게 채워줄 것인가? -> 평균값으로 채워주거나 Null인 로우를 날리는 방법도 있다..
# 데이터 전처리를 위해 학습데이터와 테스트데이터를 묶어서 관리하는 객체를 만든다.
train_test_data = [train, test]
# 이름 : 이름 자체는 별 의미가 없다.
# Mr, Mrs, Miss 등은 성별, 기혼 여부를 파악할 수 있는 데이터이기 때문에 확인을 한다.
...
# 16세 이하: 0, 17~26: 1, 27~36: 2, 37~63: 3, 63세 이상: 4
for dataset in train_test_data:
...
train['Cabin'].fillna(test.groupby('Pclass')['Cabin'].transform('median'), inplace=True)
